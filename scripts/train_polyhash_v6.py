#!/usr/bin/env python3
"""Train PolyHash v6 — parallel scan + massive hash tables for low asymptote."""
...
from chronohorn.families.polyhash.models.polyhash_v6 import PolyHashV6, V6Config
...
def main():
    ...
    p.add_argument("--scan-heads", type=int, default=1)
    p.add_argument("--scan-chunk", type=int, default=32)
    # v6b bottleneck ablation flags
    p.add_argument("--timescale-mode", default="learned")
    p.add_argument("--timescale-groups", type=int, default=1)
    p.add_argument("--cross-dim-mix", type=int, default=0)
    p.add_argument("--scan-pipeline", default="default")
    p.add_argument("--outer-product-dim", type=int, default=0)
    p.add_argument("--binding-mode", default="outer")
    # Regularization
    p.add_argument("--dropout", type=float, default=0.0)
    p.add_argument("--hash-dropout", type=float, default=0.0)
    p.add_argument("--scan-dropout", type=float, default=0.0)
    ...
    config = V6Config(
        ...
        match_offsets=mo, scan_dim=args.scan_dim, scan_heads=args.scan_heads,
        scan_chunk_size=args.scan_chunk,
        dropout=args.dropout, hash_dropout=args.hash_dropout,
        scan_dropout=args.scan_dropout,
        max_seq_len=args.seq_len, init_seed=args.seed,
        timescale_mode=args.timescale_mode, timescale_groups=args.timescale_groups,
        cross_dim_mix=args.cross_dim_mix, scan_pipeline=args.scan_pipeline,
        outer_product_dim=args.outer_product_dim,
        binding_mode=args.binding_mode,
    )
    model = PolyHashV6(config).to(device)
    ...
    result = {"model": {"test_bpb": fbpb, "test_bits_per_token": fbpt, "test_eval_loss": al,
        "params": params, "architecture": "polyhash_v6"},

#!/usr/bin/env python3
"""Train PolyHash v8 — Parallel Decepticon."""
...
def main():
    ...
    # Scale attention
    p.add_argument("--no-scale-attn", action="store_true")
    # Ablation: limit scales
    ...
    config = V8Config(
        ...
        num_scales=num_scales, buckets_per_scale=args.buckets,
        embed_per_scale=args.embed_dim,
        condition_long_on_short=not args.no_condition,
        condition_boundary=min(args.condition_boundary, num_scales),
        sign_bits=args.sign_bits,
        scale_attention=not args.no_scale_attn,
    ...
    model = PolyHashV8(config).to(device)
    params = sum(pp.numel() for pp in model.parameters())
    print(f"Model: {params:,} params ({params*6/8/1024/1024:.2f} MB int6)")
    print(f"Scales: {num_scales} (windows: {', '.join(str(SCALE_WINDOWS[i]) for i in range(num_scales))})")
    print(f"Conditioning: {'on' if config.condition_long_on_short else 'off'} (boundary={config.condition_boundary})")
    print(f"Scale attention: {'on' if config.scale_attention else 'off'}")
    ...
    result = {"model": {"test_bpb": fbpb, "test_bits_per_token": fbpt, "test_eval_loss": al,
        "params": params, "architecture": "polyhash_v8"},
        "config": {"train": {"steps": args.steps, "seq_len": args.seq_len,
            "batch_size": args.batch_size, "learning_rate": args.lr,
            "num_scales": num_scales, "buckets_per_scale": args.buckets,
            "embed_per_scale": args.embed_dim, "hidden_dim": args.hidden,
            "condition_long_on_short": config.condition_long_on_short,
            "condition_boundary": config.condition_boundary,

#!/usr/bin/env python3
"""Train PolyHash v11 — Million-bucket architecture."""
...
from chronohorn.families.polyhash.models.polyhash_v11 import PolyHashV11, V11Config
...
def main():
    ...
    p.add_argument("--buckets", type=int, default=2000000)
    p.add_argument("--embed-dim", type=int, default=2)
    p.add_argument("--byte-embed", type=int, default=64)
    ...
    p.add_argument("--match-offsets", default="1,2,3,4,5,6,7,8")
    ...
    config = V11Config(
        ...
        scan_dim=args.scan_dim if not args.no_scan else 0,
    ...
    model = PolyHashV11(config).to(device)
    params = sum(pp.numel() for pp in model.parameters())
    table_params = sum(pp.numel() for pp in model.pyramid.tables.parameters())
    other_params = params - table_params
    print(f"Model: {params:,} params ({params*6/8/1024/1024:.2f} MB int6)")
    print(f"  Tables: {table_params:,} ({table_params*6/8/1024/1024:.1f} MB) — {config.num_scales} × {config.buckets_per_scale:,} × {config.embed_per_scale}d")
    print(f"  Neural: {other_params:,} ({other_params*6/8/1024/1024:.1f} MB) — scan{config.scan_dim}d + h{config.hidden_dim}×{config.num_layers}")
    ...
    result = {"model": {"test_bpb": fbpb, "test_bits_per_token": fbpt, "test_eval_loss": al,
        "params": params, "architecture": "polyhash_v11"},
        "config": {"train": {"steps": args.steps, "seq_len": args.seq_len,
            "batch_size": args.batch_size, "learning_rate": args.lr,
            "num_scales": config.num_scales, "buckets_per_scale": config.buckets_per_scale,
            "embed_per_scale": config.embed_per_scale, "hidden_dim": config.hidden_dim,
            "num_layers": config.num_layers, "scan_dim": config.scan_dim}},

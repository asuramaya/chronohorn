#!/usr/bin/env python3
"""Train PolyHash v8m — Multi-hash Decepticon."""
...
from chronohorn.families.polyhash.models.polyhash_v8m import PolyHashV8M, V8MConfig
...
def main():
    ...
    p.add_argument("--hashes-per-scale", type=int, default=4)
    ...
    config = V8MConfig(
        ...
        embed_per_scale=args.embed_dim, hashes_per_scale=args.hashes_per_scale,
    ...
    model = PolyHashV8M(config).to(device)
    params = sum(pp.numel() for pp in model.parameters())
    print(f"Model: {params:,} params ({params*6/8/1024/1024:.2f} MB int6)")
    print(f"Scales: {num_scales}, Hashes/scale: {args.hashes_per_scale}, Buckets/hash: {config.buckets_per_scale // args.hashes_per_scale}")
    ...
    result = {"model": {"test_bpb": fbpb, "test_bits_per_token": fbpt, "test_eval_loss": al,
        "params": params, "architecture": "polyhash_v8m"},
        "config": {"train": {"steps": args.steps, "seq_len": args.seq_len,
            "batch_size": args.batch_size, "learning_rate": args.lr,
            "num_scales": num_scales, "hashes_per_scale": args.hashes_per_scale,
            "buckets_per_hash": config.buckets_per_scale // args.hashes_per_scale,

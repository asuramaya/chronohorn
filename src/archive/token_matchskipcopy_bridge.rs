...
use super::token_copy_bridge::{TokenCopyBridgeRunner, train_token_copy_bridge_from_data_root};
...
#[derive(Debug, Clone)]
pub struct TokenMatchSkipCopyBridgeReport {
    ...
    pub eval_top1_consensus_rate: f64,
}

#[derive(Debug, Clone)]
pub struct TrainedTokenMatchSkipCopyBridge {
    report: TokenMatchSkipCopyBridgeReport,
    runner: TokenMatchSkipCopyBridgeRunner,
    eval_tokens: Vec<usize>,
}

impl TrainedTokenMatchSkipCopyBridge {
    pub fn report(&self) -> &TokenMatchSkipCopyBridgeReport {
        &self.report
    }

    pub fn runner(&self) -> &TokenMatchSkipCopyBridgeRunner {
        &self.runner
    }
...
#[derive(Debug, Clone)]
pub struct TokenMatchSkipCopyBridgeRunner {
...
pub fn train_token_matchskipcopy_bridge_from_data_root(
    root: &Path,
    ...
    alpha_skip: f64,
    copy_decay: f64,
) -> Result<TrainedTokenMatchSkipCopyBridge, String> {
    if train_stride == 0 {
    ...
    if trained_match.runner().vocab_size() != trained_skip.runner().vocab_size()
        || trained_match.runner().vocab_size() != trained_copy.runner().vocab_size()
    {
        return Err("match/skip/copy vocab size mismatch".to_string());
    }
    ...
    if train_raw.is_empty() || tune_raw.is_empty() || eval_raw.is_empty() {
        return Err("no matchskipcopy records collected".to_string());
    }
    ...
    let direct_model = train_direct_gate(&train_records, 80, 0.08, 1e-4);

    let tuned_heuristic_lambda = tune_lambda(&tune_records, |record| record.heuristic_logits);
    let tuned_direct_lambda = tune_lambda(&tune_records, |record| {
        predict_logits(&direct_model, &record.features)
    });
    ...
    let report = TokenMatchSkipCopyBridgeReport {
        train_token_budget,
        trigram_buckets,
        skip_buckets,
        ...
        eval_top1_consensus_rate: consensus_rate(&eval_records),
    };

    let runner = TokenMatchSkipCopyBridgeRunner {
    ...
    Ok(TrainedTokenMatchSkipCopyBridge {
        report,
        runner,
        eval_tokens,
    })
...
pub fn run_token_matchskipcopy_bridge_from_data_root(
    root: &Path,
    train_token_budget: usize,
    trigram_buckets: usize,
    skip_buckets: usize,
    ...
    alpha_skip: f64,
    copy_decay: f64,
) -> Result<TokenMatchSkipCopyBridgeReport, String> {
    Ok(train_token_matchskipcopy_bridge_from_data_root(
        root,
        train_token_budget,
        trigram_buckets,
        skip_buckets,
...
pub fn render_token_matchskipcopy_bridge_report(report: &TokenMatchSkipCopyBridgeReport) -> String {
    let mut out = String::new();
    out.push_str("chronohorn_token_matchskipcopy_bridge\n");
    out.push_str(&format!(
        "train_token_budget: {}\n",
    ...
    out.push_str(&format!(
        "eval_top1_consensus_rate: {:.6}\n",
        report.eval_top1_consensus_rate
    ));
    out
}

impl Runner for TokenMatchSkipCopyBridgeRunner {
    fn name(&self) -> &'static str {
        "TokenMatchSkipCopyBridgeRunner"
    }
    ...
    fn score_chunk(
        ...
        if match_outputs.sample_predictions.len() != skip_outputs.sample_predictions.len()
            || match_outputs.sample_predictions.len() != copy_outputs.sample_predictions.len()
        {
            return Err("match/skip/copy sample prediction length mismatch".to_string());
        }
        ...
        for (index, ((match_dist, skip_dist), copy_dist)) in match_outputs
            .sample_predictions
            .iter()
            ...
            .enumerate()
        {
            ...
            let copy_stats = summarize_distribution(copy_dist, self.candidate_k);
            let features = standardize_features(
                extract_features(&match_stats, &skip_stats, &copy_stats),
                &self.standardizer,
            );
            let raw_logits = match &self.gate {
                GateChoice::Heuristic => {
                    heuristic_logits_from_stats(&match_stats, &skip_stats, &copy_stats)
                }
                GateChoice::Direct(model) => predict_logits(model, &features),
            };
            let weights = softmax3(scale_logits(raw_logits, self.lambda));
            let mixed = mix_three(match_dist, skip_dist, copy_dist, weights);
            let gold = mixed
...
fn collect_raw_records(
    ...
    for row_index in 0..sample_positions.len() {
        let match_stats =
            summarize_distribution(&match_outputs.sample_predictions[row_index], candidate_k);
        let skip_stats =
            summarize_distribution(&skip_outputs.sample_predictions[row_index], candidate_k);
        let copy_stats =
            summarize_distribution(&copy_outputs.sample_predictions[row_index], candidate_k);
        rows.push(RawRecord {
            features: extract_features(&match_stats, &skip_stats, &copy_stats),
            match_gold_prob: match_golds[row_index].exp().max(1e-12),
            skip_gold_prob: skip_golds[row_index].exp().max(1e-12),
            copy_gold_prob: copy_golds[row_index].exp().max(1e-12),
            heuristic_logits: heuristic_logits_from_stats(&match_stats, &skip_stats, &copy_stats),
            consensus_top1: match_stats.top1_token == skip_stats.top1_token
                && skip_stats.top1_token == copy_stats.top1_token,
        });
    }
    Ok(rows)
}
...
fn standardize_records(records: &[RawRecord], standardizer: &Standardizer) -> Vec<Record> {
    records
        ...
        .map(|record| Record {
            ...
            heuristic_logits: record.heuristic_logits,
            consensus_top1: record.consensus_top1,
        })
        .collect()
}
...
fn extract_features(
    ...
    copy_stats: &DistStats,
) -> [f64; FEATURE_DIM] {
    let ms = if match_stats.top1_token == skip_stats.top1_token {
        1.0
    } else {
        0.0
    };
    let mc = if match_stats.top1_token == copy_stats.top1_token {
        1.0
    } else {
        0.0
    };
    let sc = if skip_stats.top1_token == copy_stats.top1_token {
        1.0
    } else {
        0.0
    };
...
fn heuristic_logits_from_stats(
    match_stats: &DistStats,
    skip_stats: &DistStats,
    copy_stats: &DistStats,
) -> [f64; 3] {
    ...
    [
        match_stats.top1_prob + match_stats.topk_mass - match_stats.entropy_norm + 0.25 * (ms + mc),
        skip_stats.top1_prob + skip_stats.topk_mass - skip_stats.entropy_norm + 0.25 * (ms + sc),
        copy_stats.top1_prob + copy_stats.topk_mass - copy_stats.entropy_norm + 0.25 * (mc + sc),
    ]
}

fn train_direct_gate(records: &[Record], epochs: usize, lr: f64, l2: f64) -> MultiModel {
    let mut model = MultiModel {
        weights: [[0.0; FEATURE_DIM]; 3],
        bias: [0.0; 3],
    };
    for _ in 0..epochs {
        for record in records {
            let label = if record.match_gold_prob >= record.skip_gold_prob
                && record.match_gold_prob >= record.copy_gold_prob
            {
                0
            } else if record.skip_gold_prob >= record.copy_gold_prob {
                1
            } else {
                2
            };
            let logits = predict_logits(&model, &record.features);
            let probs = softmax3(logits);
            for class in 0..3 {
                let target = if class == label { 1.0 } else { 0.0 };
                let error = probs[class] - target;
                for index in 0..FEATURE_DIM {
                    model.weights[class][index] -=
                        lr * (error * record.features[index] + l2 * model.weights[class][index]);
                }
                model.bias[class] -= lr * error;
            }
        }
    }
    model
...
fn predict_logits(model: &MultiModel, features: &[f64; FEATURE_DIM]) -> [f64; 3] {
    let mut logits = [0.0; 3];
    for class in 0..3 {
        let mut value = model.bias[class];
        for index in 0..FEATURE_DIM {
            value += model.weights[class][index] * features[index];
        }
        logits[class] = value;
    }
    logits
...
fn scale_logits(logits: [f64; 3], lambda: f64) -> [f64; 3] {
    [logits[0] * lambda, logits[1] * lambda, logits[2] * lambda]
}
...
fn mean_bits_for_records(
    records: &[Record],
    gate: Option<(f64, GateKind, Option<&MultiModel>)>,
    branch: Branch,
) -> f64 {
    records
        .iter()
        .map(|record| {
            let prob = match branch {
                ...
                Branch::Combined => {
                    ...
                    let logits = match gate_kind {
                        GateKind::Heuristic => record.heuristic_logits,
                        GateKind::Direct => {
                            predict_logits(model.expect("missing direct model"), &record.features)
                        }
                    };
                    let weights = softmax3(scale_logits(logits, lambda));
                    (weights[0] * record.match_gold_prob
                        + weights[1] * record.skip_gold_prob
                        + weights[2] * record.copy_gold_prob)
                        .max(1e-12)
                }
            };
            -prob.max(1e-12).log2()
    ...
    F: FnMut(&Record) -> [f64; 3],
{
    records
        .iter()
        .map(|record| {
            let logits = gate_fn(record);
            let weights = softmax3(scale_logits(logits, lambda));
            let prob = (weights[0] * record.match_gold_prob
                + weights[1] * record.skip_gold_prob
                + weights[2] * record.copy_gold_prob)
                .max(1e-12);
            -prob.log2()
...
fn branch_better_rate(records: &[Record], branch: Branch) -> f64 {
    records
        ...
        .filter(|record| match branch {
            Branch::Match => {
                record.match_gold_prob > record.skip_gold_prob
                    && record.match_gold_prob > record.copy_gold_prob
            }
            Branch::Skip => {
                record.skip_gold_prob > record.match_gold_prob
                    && record.skip_gold_prob > record.copy_gold_prob
            }
            Branch::Copy => {
                record.copy_gold_prob > record.match_gold_prob
                    && record.copy_gold_prob > record.skip_gold_prob
            }
...
fn consensus_rate(records: &[Record]) -> f64 {
    records
        .iter()
        .filter(|record| record.consensus_top1)
        .count() as f64
        / records.len() as f64
}
...
fn mix_three(
    ...
    let mut total = 0.0;
    for ((&m, &s), &c) in match_dist
        .iter()
        .zip(skip_dist.iter())
        .zip(copy_dist.iter())
    {
        let value = weights[0] * m + weights[1] * s + weights[2] * c;
        mixed.push(value);
        total += value;
    }
    if total > 0.0 {

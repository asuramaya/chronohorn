...
**`eval.knn_datastore`** (`python/chronohorn/eval/knn_datastore.py`) — the census state-kNN eval as parametrized architecture (subsumes all three `experiments/harnesses/knn_*.py` scripts). Composes the decepticons kernel organs (`LinearStateStreamer` + `StateKNNMemory`); store size, RAM tier, states mode, and key transform are config. Run: `python -m chronohorn.eval.knn_datastore --store-bytes 8000000` (add `--store-device cpu` for the RAM key store past the VRAM ceiling; `--states-mode windowed` for per-window forward states instead of continuous-stream; `--arm pca128 jl128 pca64` to sweep key transforms). Shared eval helpers in `eval/harness_util.py`.
